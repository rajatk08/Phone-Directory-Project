...
class contactBook{
    ...
    void CreateNode(){
        if(head==NULL){
            Node * newer=new Node;
            cout<<"Name of contact to be added: ";
            cin>>x;
            newer->name=x;
    ...
    void Display(){
        ...
        if (temp==NULL){
            ...
            while(temp!=NULL){
                cnt++;
                cout<<"\t"<<temp->name<<"\t\t"<<temp->phNo<<endl<<endl;

                temp=temp->next;
            }
            cout<<endl<<"Total contacts: "<<cnt<<endl;
        }
    ...
    void DeleteContactBySearch(){
	    ...
	    cout<<"Press 2 if you want to Search By Number"<<endl;
		int command;
    	cout<<"Enter the Command: ";	    	cin>>command;

        if(command==1){
	    	bool Dcheck=false;
	    	cout<<"Enter the Contact's Name to Delete: ";
			cin>>x;
